...
func TestExactStalenessTransaction(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		var minStartTS uint64
		if !testcase.IsStaleness {
			var err error
			minStartTS, err = store.GetOracle().GetTimestamp(context.Background(), &oracle.Option{})
		...
		if testcase.expectPhysicalTS > 0 {
			...
			maxStartTS, err := store.GetOracle().GetTimestamp(context.Background(), &oracle.Option{})
			require.NoError(t, err)
			require.GreaterOrEqual(t, startTS, minStartTS)
			require.LessOrEqual(t, startTS, maxStartTS)
		}
		tk.MustExec("commit")
	}
