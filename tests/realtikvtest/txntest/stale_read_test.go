...
func TestExactStalenessTransaction(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		var beforeTxnTS uint64
		if !testcase.IsStaleness {
			var err error
			beforeTxnTS, err = store.GetOracle().GetTimestamp(context.Background(), &oracle.Option{TxnScope: oracle.GlobalTxnScope})
		...
		if testcase.expectPhysicalTS > 0 {
			...
			afterTxnTS, err := store.GetOracle().GetTimestamp(context.Background(), &oracle.Option{TxnScope: oracle.GlobalTxnScope})
			require.NoError(t, err)
			startTS := tk.Session().GetSessionVars().TxnCtx.StartTS
			require.Greater(t, startTS, beforeTxnTS)
			require.Less(t, startTS, afterTxnTS)
		}
		tk.MustExec("commit")
	}
