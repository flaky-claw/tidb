...
func TestExactStalenessTransaction(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		var normalTxnStartTSLowerBound uint64
		if !testcase.IsStaleness {
			var err error
			normalTxnStartTSLowerBound, err = store.GetOracle().GetTimestamp(
				context.Background(),
		...
		if testcase.expectPhysicalTS > 0 {
			...
			normalTxnStartTSUpperBound, err := store.GetOracle().GetTimestamp(
				context.Background(),
				&oracle.Option{TxnScope: oracle.GlobalTxnScope},
			)
			require.NoError(t, err)
			startTS := tk.Session().GetSessionVars().TxnCtx.StartTS
			require.GreaterOrEqual(t, startTS, normalTxnStartTSLowerBound)
			require.LessOrEqual(t, startTS, normalTxnStartTSUpperBound)
		}
		tk.MustExec("commit")
	}
