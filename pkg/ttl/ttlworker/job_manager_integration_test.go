...
func TestTTLPKFallbackPlannerContracts(t *testing.T) {
	...
	runTTLScanPlannerContract(t, store, tk, dom, "ttl_pk_plan_1",
	...
	runTTLScanPlannerContract(t, store, tk, dom, "ttl_pk_fallback_2",
...
func runTTLScanPlannerContract(t *testing.T, store kv.Storage, tk *testkit.TestKit, dom *domain.Domain, tableName string, createTableSQL string) {
	...
	probeTK := testkit.NewTestKit(t, store)
	var probeOnce sync.Once
	testfailpoint.EnableCall(t, "github.com/pingcap/tidb/pkg/statistics/handle/storage/beforeHistogramFromStorageWithParams",
		func(int64, bool, int64) {
			probeOnce.Do(func() {
				probeTK.MustQuery("select count(*) from mysql.stats_histograms").Rows()
			})
		})
