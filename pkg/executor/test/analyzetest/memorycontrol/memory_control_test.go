...
func setGlobalAnalyzeMemQuotaForTest(t *testing.T, tk *testkit.TestKit, quota int64) {
	t.Helper()
	oldQuota := executor.GlobalAnalyzeMemoryTracker.GetBytesLimit()
	tk.MustExec(fmt.Sprintf("set global tidb_mem_quota_analyze=%d", quota))
	t.Cleanup(func() {
		tk.MustExec(fmt.Sprintf("set global tidb_mem_quota_analyze=%d", oldQuota))
	})
}
...
func TestMemQuotaAnalyze(t *testing.T) {
	...
	setGlobalAnalyzeMemQuotaForTest(t, tk, 128)
